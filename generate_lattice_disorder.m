...
ainbohr = alat*ang2bohr;
max_onsite = exp(-R0*ainbohr)/(2*ainbohr*epsilon);
onsite_energies = zeros(1,L^2);
...
emax = fac * max_onsite;
onsite_energies = (randn(1,L^2) - 0.5)*emax;
save(join(['disorder_','tri_',num2str(L^2),'_',num2str(seed),'_',num2str(regime)],'onsite_energies'))
end
