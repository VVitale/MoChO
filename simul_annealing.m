...
function [charge_state_best,ics_best,E_min,Ehist,potential,mcell,...
    onsite_energies] = simul_annealing(alat,tfac,max_iter,L,restart,...
    nrep,epsilon,Treal,ncells,filling,disorder,add_disorder,R0)
   
   % Useful conversion factors
   ang2bohr = 1.889725989; 
   alat = alat*ang2bohr; % Bohr
   kb =  8.617333262145e-5 / 27.211386246 ;% Hartree K^-1;
   ...
   a1 = alat*[1,0];%[1/2,sqrt(3)/2,0];
   a2 = alat*[1/2,sqrt(3)/2];%[1/2,-sqrt(3)/2,0];
   a3 = a2 - a1;
   % Reciprocal vectors
   ...
   for in = 0 : L-1
      for jn = 0 : L-1
          mcell(il,:) = in*a1 + jn*a2;
          onsite_energies(il) = 0.0;
%           mcell(il+1,:) = 2/3*(a1+a2) + in*a1+jn*a2;
%           onsite_energies(il+1) = 0.0;
%           il = il + 2;
          il = il + 1;
      end
   end
          ...
          for ia = -1 : 1
             for ib = -1 : 1
                ...
                if(dist_ij~=0 && abs(dist_ij-alat)<0.01)
                   ...
                   nn(in,inn) = jn;
                elseif(dist_ij > alat/sqrt(3) && abs(dist_ij - alat*sqrt(3))<0.1)
                    innn = innn+1;
                    if(innn > 6)
   ...
   potential = real_pot2(nsites,epsilon,L*alat/2,ma1,ma2,mcell,R0);
       ...
       while temp_iter < nrep
           ...
           while isempty(ineigh)
               ...
               if(crand <= 0.5)
                   ...
                   % unoccupied site
                   ineigh = find(~charge_state(nn(itemp,:))); 
               else
                   ineigh = find(~charge_state(nnn(itemp,:)));
               end
           end
           if(crand <= 0.5)
               ineigh_temp = randsample(nn(itemp,ineigh),1);
           else
               ineigh_temp = randsample(nnn(itemp,ineigh),1);
           end
